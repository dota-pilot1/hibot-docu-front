...
const initialState: Omit<
  UserState,
  "setUser" | "setAccessToken" | "triggerLoginFocus" | "logout" | "hydrate"
> = {
  user: null,
  accessToken: null,
  loginFocusTrigger: 0,
...
const loadPersistedState = () => {
  ...
  try {
    ...
    if (stored) {
      ...
      // userId가 유효하지 않으면 (undefined, null, NaN, 0) 로그아웃 처리
      if (
        user &&
        (!user.userId || typeof user.userId !== "number" || isNaN(user.userId))
      ) {
        console.warn("Invalid userId detected, clearing auth data");
...
export const userStore = new Store<UserState>({
  ...
  triggerLoginFocus: () => {
    userStore.setState((state) => ({
      ...state,
      loginFocusTrigger: state.loginFocusTrigger + 1,
    }));
  },
  logout: () => {
    userStore.setState((state) => ({
