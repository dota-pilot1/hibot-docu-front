...
import { usePilotTree } from "../model/usePilotTree";
import { usePilotContents } from "../model/usePilotContents";
import {
  usePilotFiles,
  usePilotFileMutations,
} from "../model/usePilotFiles";
...
export const PilotDetailView = () => {
  ...
  } = usePilotContents(selectedCategory);
  ...
  } = usePilotFiles(selectedCategory);
  ...
  const [tempTree, setTempTree] = useState<PilotCategory[] | null>(null);
  const [tempContents, setTempContents] = useState<PilotContent[] | null>(null);
  ...
  const [editingCategory, setEditingCategory] = useState<PilotCategory | null>(
    null
  );
  const [categoryForm, setCategoryForm] = useState({
    name: "",
    parentId: null as number | null,
    pilotType: "NOTE" as PilotType,
  });
  ...
  const [editingContent, setEditingContent] = useState<PilotContent | null>(
    null
  );
  ...
  const getFilteredTree = (sourceTree: PilotCategory[]): PilotCategory[] => {
  ...
  useEffect(() => {
    if (filteredTree.length > 0) {
      const collectDepth1 = (categories: PilotCategory[]): number[] => {
        const ids: number[] = [];
        for (const cat of categories) {
          if (cat.depth === 1) {
            ids.push(cat.id);
  ...
  const selectedCategoryData = selectedCategory
    ...
    : null;
  const isFileCategory = selectedCategoryData?.pilotType === "FILE";
  ...
  const handleCategoryDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      await pilotApi.reorderCategories(categoryIds, parentId);
      refetchTree();
    } catch (error) {
  ...
  const handleContentDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      await pilotApi.reorderContents(selectedCategory, contentIds);
      refetchContents();
    } catch (error) {
  ...
  const renderTreeItem = (cat: PilotCategory) => (
    <SortableTreeItem key={cat.id} id={cat.id} isAdminMode={isAdminMode}>
      {({ attributes, listeners }) => (
        <div
          style={{ marginLeft: `${(cat.depth - 1) * 20}px` }}
          className={`group flex items-center gap-1 rounded-md transition-colors ${selectedCategory === cat.id ? "bg-green-100" : "hover:bg-gray-100"}`}
        >
          ...
          <Button
            ...
            className={`flex-1 justify-start h-8 px-2 hover:bg-transparent ${selectedCategory === cat.id ? "text-green-900" : ""}`}
          >
            {cat.children && cat.children.length > 0 ? (
              expandedIds.has(cat.id) ? (
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold">
            {project ? `${project} Projects` : "All Projects"}
          </h1>
        </div>
        <div
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="md:col-span-1">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 py-3 bg-green-100">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setContentForm({
                    ...
                    content: "",
                    contentType:
                      (selectedCategoryData?.pilotType as any) || "NOTE",
                    metadata: {},
                  });
                            ...
                            <CardHeader className="pb-2">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                  <CardTitle
                                    className="text-lg cursor-pointer hover:text-green-600 transition-colors flex items-center gap-2 truncate"
                                    onClick={() => {
                              ...
                              <CardContent>
                                ...
                                <div className="mt-2 flex justify-center border-t border-gray-50 dark:border-gray-800 pt-2">
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    className="text-green-600 hover:text-green-700 font-medium h-8 w-full"
                                    onClick={() => {
                                      setViewingContent(content);
                                      setIsViewModalOpen(true);
                                    }}
      ...
      <FormDialog
        ...
        onSubmit={async () => {
          if (categoryModalMode === "create") {
            await pilotApi.createCategory({
              name: categoryForm.name,
              parentId: categoryForm.parentId ?? undefined,
              pilotType: categoryForm.pilotType,
              projectType: project || undefined,
            });
          } else if (editingCategory) {
            await pilotApi.updateCategory(editingCategory.id, {
              name: categoryForm.name,
              pilotType: categoryForm.pilotType,
            });
          }
      ...
      <FormDialog
        ...
        onSubmit={async () => {
          if (contentModalMode === "create" && selectedCategory) {
            ...
            await pilotApi.updateContent(editingContent.id, {
              title: contentForm.title,
              content: contentForm.content,
              contentType: contentForm.contentType,
              metadata: contentForm.metadata,
            ...
            <Label
              ...
              className={
                contentForm.contentType === "QA"
                  ? "text-green-600 font-bold text-base block"
            ...
            <div className="mt-2">
              {contentForm.contentType === "QA" ? (
                <Textarea
                  ...
                  rows={4}
                  className="bg-white border-gray-300 focus:border-green-500 text-lg p-4"
                />
              ) : (
                <Input
                  id="content-title"
                  ...
                  className="bg-white border-gray-300 focus:border-green-500 h-10"
                />
              )}
            </div>
          </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (categoryToDelete) {
            await pilotApi.deleteCategory(categoryToDelete);
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (contentToDelete) {
            await pilotApi.deleteContent(contentToDelete);
            refetchContents();
      ...
      <Dialog open={isViewModalOpen} onOpenChange={setIsViewModalOpen}>
        ...
        >
          <DialogHeader className="px-6 py-3 bg-green-100 border-b border-green-200">
            <div className="flex items-center justify-between">
          ...
          <div className="flex-1 overflow-y-auto px-8 py-2 bg-white">
            {viewingContent?.contentType === "NOTE" && (
              <div className="prose prose-lg max-w-none dark:prose-invert prose-pre:bg-transparent prose-pre:p-0 prose-pre:m-0 prose-code:text-green-600 prose-code:before:content-none prose-code:after:content-none font-sans">
                <ReactMarkdown
            ...
            {viewingContent?.contentType === "QA" && (
              <div className="space-y-4">
                {viewingContent.content ? (
                  <div className="prose prose-lg max-w-none dark:prose-invert prose-pre:bg-transparent prose-pre:p-0 prose-pre:m-0 prose-code:text-green-600 prose-code:before:content-none prose-code:after:content-none">
                    <ReactMarkdown
                      remarkPlugins={[remarkGfm, remarkBreaks]}
                      components={{
                        code: MarkdownCodeBlock as any,
      ...
      <FormDialog
        ...
        onSubmit={async () => {
          if (answerContent) {
            await pilotApi.updateContent(answerContent.id, {
              content: answerText,
            });
