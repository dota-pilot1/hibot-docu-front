...
import { useNoteContents } from "../model/useNoteContents";
import { useNoteFiles, useNoteFileMutations } from "../model/useNoteFiles";
...
export const NoteDetailView = () => {
  ...
  const [isAdminMode, setIsAdminMode] = useState(true); // 개인 노트이므로 기본 Admin 모드
  const { tree, isLoading: treeLoading, refetch: refetchTree } = useNoteTree();
  const {
    contents,
    isLoading: contentsLoading,
    refetch: refetchContents,
  } = useNoteContents(selectedCategory);
  ...
  } = useNoteFiles(selectedCategory);
  const { isUploading, uploadFile, deleteFile, renameFile, downloadFile } =
    useNoteFileMutations();

  const [expandedIds, setExpandedIds] = useState<Set<number>>(new Set());

  // 드래그 중 임시 상태 (실시간 자리바꾸기용)
  const [tempTree, setTempTree] = useState<NoteCategory[] | null>(null);
  const [tempContents, setTempContents] = useState<NoteContent[] | null>(null);

  // 드래그 중일 때는 임시 상태 사용, 아니면 원본 사용
  const displayTree = tempTree ?? tree;
  const displayContents = tempContents ?? contents;

  // Filter tree based on category parameter
  const getFilteredTree = (sourceTree: NoteCategory[]): NoteCategory[] => {
    if (categoryParam && sourceTree.length > 0) {
      const findCategory = (
        categories: NoteCategory[],
      ): NoteCategory | null => {
        for (const cat of categories) {
          if (cat.id.toString() === categoryParam) {
            return cat;
          }
      ...
      const targetCategory = findCategory(sourceTree);
      if (targetCategory) {
  ...
  const parentCategory = filteredTree[0] || null;
  ...
  const [editingContent, setEditingContent] = useState<NoteContent | null>(
    null,
  );
  ...
  // Initialize expandedIds with all items when tree changes
  useEffect(() => {
    if (tree.length > 0) {
      const collectAllIds = (categories: NoteCategory[]): number[] => {
        const ids: number[] = [];
        for (const cat of categories) {
          ids.push(cat.id);
          if (cat.children && cat.children.length > 0) {
  ...
  const selectedCategoryData = selectedCategory
    ...
    : null;
  const isFileCategory = selectedCategoryData?.noteType === "FILE";
  ...
  const handleCategoryDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      await noteApi.reorderCategories(categoryIds, parentId);
      refetchTree();
    } catch (error) {
  ...
  const handleContentDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      await noteApi.reorderContents(selectedCategory, contentIds);
      refetchContents();
    } catch (error) {
  ...
  const renderTreeItem = (cat: NoteCategory) => (
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">
              {parentCategory ? parentCategory.name : "My Notes"}
            </h1>
            <p className="text-sm text-gray-500">
              {parentCategory
                ? parentCategory.description || "노트 상세"
                : "개인 노트 관리"}
            </p>
          </div>
        </div>
            ...
            {treeLoading ? (
              ...
              <div className="text-center py-8">
                ...
                {isAdminMode && (
                  ...
                  >
                    <Plus className="h-4 w-4 mr-1" /> 첫 노트 추가
                  </Button>
                )}
              </div>
            ) : (
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setContentForm({
                    ...
                    content: "",
                    contentType:
                      (selectedCategoryData?.noteType as any) || "NOTE",
                    metadata: {},
                  });
        ...
        onSubmit={async () => {
          if (categoryModalMode === "create") {
            await noteApi.createCategory({
              ...
              noteType: categoryForm.noteType,
            });
          } else if (editingCategory) {
            await noteApi.updateCategory(editingCategory.id, {
              name: categoryForm.name,
              noteType: categoryForm.noteType,
            });
          }
      ...
      <FormDialog
        ...
        onSubmit={async () => {
          if (contentModalMode === "create" && selectedCategory) {
            ...
            await noteApi.updateContent(editingContent.id, {
              title: contentForm.title,
              content: contentForm.content,
              contentType: contentForm.contentType,
              metadata: contentForm.metadata,
            ...
            <div className="mt-2">
              {contentForm.contentType === "QA" ? (
                <textarea
                  ...
                  rows={4}
                  className="w-full bg-white border border-gray-300 focus:border-blue-500 text-lg p-4 rounded-md"
                />
              ) : (
                <Input
                  id="content-title"
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (categoryToDelete) {
            await noteApi.deleteCategory(categoryToDelete);
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (contentToDelete) {
            await noteApi.deleteContent(contentToDelete);
            refetchContents();
      ...
      <FormDialog
        ...
        onSubmit={async () => {
          if (answerContent) {
            await noteApi.updateContent(answerContent.id, {
              content: answerText,
            });
