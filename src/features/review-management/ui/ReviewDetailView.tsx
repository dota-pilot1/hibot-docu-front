...
import { useReviewTree } from "../model/useReviewTree";
import { useReviewContents } from "../model/useReviewContents";
import {
  useReviewFiles,
  useReviewFileMutations,
} from "../model/useReviewFiles";
...
export const ReviewDetailView = () => {
  ...
  } = useReviewContents(selectedCategory);
  ...
  } = useReviewFiles(selectedCategory);
  ...
  const [tempTree, setTempTree] = useState<ReviewCategory[] | null>(null);
  const [tempContents, setTempContents] = useState<ReviewContent[] | null>(null);
  ...
  const [editingCategory, setEditingCategory] = useState<ReviewCategory | null>(
    null
  );
  const [categoryForm, setCategoryForm] = useState({
    name: "",
    parentId: null as number | null,
    reviewType: "NOTE" as ReviewType,
  });
  ...
  const [editingContent, setEditingContent] = useState<ReviewContent | null>(
    null
  );
  ...
  const getFilteredTree = (sourceTree: ReviewCategory[]): ReviewCategory[] => {
  ...
  useEffect(() => {
    if (filteredTree.length > 0) {
      const collectDepth1 = (categories: ReviewCategory[]): number[] => {
        const ids: number[] = [];
        for (const cat of categories) {
          if (cat.depth === 1) {
            ids.push(cat.id);
  ...
  const selectedCategoryData = selectedCategory
    ...
    : null;
  const isFileCategory = selectedCategoryData?.reviewType === "FILE";
  ...
  const handleCategoryDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      await reviewApi.reorderCategories(categoryIds, parentId);
      refetchTree();
    } catch (error) {
  ...
  const handleContentDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      await reviewApi.reorderContents(selectedCategory, contentIds);
      refetchContents();
    } catch (error) {
  ...
  const renderTreeItem = (cat: ReviewCategory) => (
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold">
            {project ? `${project} Reviews` : "All Reviews"}
          </h1>
        </div>
        <div
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="md:col-span-1">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 py-3 bg-blue-100">
            <CardTitle>Review Tree</CardTitle>
          </CardHeader>
          <CardContent>
            {treeLoading ? (
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setContentForm({
                    ...
                    content: "",
                    contentType:
                      (selectedCategoryData?.reviewType as any) || "NOTE",
                    metadata: {},
                  });
      ...
      <FormDialog
        ...
        onSubmit={async () => {
          if (categoryModalMode === "create") {
            await reviewApi.createCategory({
              name: categoryForm.name,
              parentId: categoryForm.parentId ?? undefined,
              reviewType: categoryForm.reviewType,
              projectType: project || undefined,
            });
          } else if (editingCategory) {
            await reviewApi.updateCategory(editingCategory.id, {
              name: categoryForm.name,
              reviewType: categoryForm.reviewType,
            });
          }
      ...
      <FormDialog
        ...
        onSubmit={async () => {
          if (contentModalMode === "create" && selectedCategory) {
            ...
            await reviewApi.updateContent(editingContent.id, {
              title: contentForm.title,
              content: contentForm.content,
              contentType: contentForm.contentType,
              metadata: contentForm.metadata,
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (categoryToDelete) {
            await reviewApi.deleteCategory(categoryToDelete);
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (contentToDelete) {
            await reviewApi.deleteContent(contentToDelete);
            refetchContents();
      ...
      <Dialog open={isViewModalOpen} onOpenChange={setIsViewModalOpen}>
        ...
        >
          <DialogHeader className="px-6 py-3 bg-blue-100 border-b border-blue-200">
            <div className="flex items-center justify-between">
      ...
      <FormDialog
        ...
        onSubmit={async () => {
          if (answerContent) {
            await reviewApi.updateContent(answerContent.id, {
              content: answerText,
            });
