...
import { useDesignSystemMutations } from "../model/useDesignSystemMutations";
...
import type { DesignSystemCategory } from "@/entities/design-system/model/types";
import { PlusIcon, TrashIcon } from "lucide-react";

export const DesignSystemMatrix = () => {
  ...
  const { createCategory, deleteCategory } = useDesignSystemMutations();

  const fetchData = async () => {
    setIsLoading(true);
    setError(null);
    try {
      console.log("Fetching design system tree...");
      const data = await designSystemApi.getTree();
  ...
  const handleAddGroup = async () => {
    ...
    try {
      await createCategory({ name: groupName, designSystemType: "NOTE" });
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <p className="text-center text-gray-500">Loading design systems...</p>
        {[...Array(3)].map((_, i) => (
  ...
  const renderCategory = (category: DesignSystemCategory) => {
        ...
        <Card className="border-gray-200 shadow-sm">
          <CardContent className="py-12">
            <p className="text-center text-gray-400">
              디자인 시스템이 없습니다. Admin 모드를 켜고 그룹을 추가해주세요.
            </p>
          </CardContent>
        </Card>
      ) : (
