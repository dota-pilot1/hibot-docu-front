...
import { useDesignSystemTree } from "../model/useDesignSystemTree";
import { useDesignSystemContents } from "../model/useDesignSystemContents";
import {
  useDesignSystemFiles,
  useDesignSystemFileMutations,
} from "../model/useDesignSystemFiles";
...
export const DesignSystemDetailView = () => {
  ...
  } = useDesignSystemContents(selectedCategory);
  ...
  } = useDesignSystemFiles(selectedCategory);
  ...
  const [editingCategory, setEditingCategory] =
    useState<DesignSystemCategory | null>(null);
  ...
  const getFilteredTree = (
    ...
    if (tech && sourceTree.length > 0) {
      ...
      const getAllChildren = (
        ...
        let result: DesignSystemCategory[] = [];
        for (const cat of categories) {
          if (cat.children && cat.children.length > 0) {
  ...
  useEffect(() => {
    if (filteredTree.length > 0) {
      const collectDepth1 = (categories: DesignSystemCategory[]): number[] => {
        const ids: number[] = [];
        for (const cat of categories) {
          if (cat.depth === 1) {
            ids.push(cat.id);
  ...
  const findCategoryById = (
    nodes: DesignSystemCategory[],
    id: number,
  ): DesignSystemCategory | undefined => {
  ...
  const replaceSiblings = (
    nodes: DesignSystemCategory[],
    targetId: number,
    newSiblings: DesignSystemCategory[],
  ): DesignSystemCategory[] => {
    // 현재 레벨에서 찾으면 교체
  ...
  const handleCategoryDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      await designSystemApi.reorderCategories(categoryIds, parentId);
      refetchTree();
    } catch (error) {
  ...
  const handleContentDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      await designSystemApi.reorderContents(selectedCategory, contentIds);
      refetchContents();
    } catch (error) {
  ...
  const renderTreeItem = (cat: DesignSystemCategory) => (
  ...
  return (
    <div className="space-y-4">
      ...
      <FormDialog
        ...
        onSubmit={async () => {
          if (contentModalMode === "create" && selectedCategory) {
            await designSystemApi.createContent({
            ...
            await designSystemApi.updateContent(editingContent.id, {
              title: contentForm.title,
              content: contentForm.content,
              contentType: contentForm.contentType,
              metadata: contentForm.metadata,
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (categoryToDelete) {
            await designSystemApi.deleteCategory(categoryToDelete);
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (contentToDelete) {
            await designSystemApi.deleteContent(contentToDelete);
            refetchContents();
      ...
      <FormDialog
        ...
        onSubmit={async () => {
          if (answerContent) {
            await designSystemApi.updateContent(answerContent.id, {
              content: answerText,
            });
