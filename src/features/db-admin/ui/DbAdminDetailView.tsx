...
import { useDbAdminTree } from "../model/useDbAdminTree";
import { useDbAdminContents } from "../model/useDbAdminContents";
import {
  useDbAdminFiles,
  useDbAdminFileMutations,
} from "../model/useDbAdminFiles";
...
export const DbAdminDetailView = () => {
  ...
  } = useDbAdminContents(selectedCategory);
  ...
  } = useDbAdminFiles(selectedCategory);
  ...
  const [tempTree, setTempTree] = useState<DbAdminCategory[] | null>(null);
  const [tempContents, setTempContents] = useState<DbAdminContent[] | null>(null);
  ...
  const [editingCategory, setEditingCategory] = useState<DbAdminCategory | null>(
    null
  );
  const [categoryForm, setCategoryForm] = useState({
    name: "",
    parentId: null as number | null,
    dbAdminType: "NOTE" as DbAdminType,
  });
  ...
  const [editingContent, setEditingContent] = useState<DbAdminContent | null>(
    null
  );
  ...
  // Filter tree: find the selected project and show it as root (including its children)
  const getFilteredTree = (sourceTree: DbAdminCategory[]): DbAdminCategory[] => {
    if (project && sourceTree.length > 0) {
      // Find the project category in the tree
      const findProject = (
        categories: DbAdminCategory[]
      ): DbAdminCategory | null => {
        for (const cat of categories) {
          if (cat.projectType === project || cat.id.toString() === project) {
            return cat;
          }
          if (cat.children && cat.children.length > 0) {
            const found = findProject(cat.children);
      ...
      const projectCategory = findProject(sourceTree);
      if (projectCategory) {
        return [projectCategory]; // Show the project category itself as root
      }
      return [];
    } else {
  ...
  useEffect(() => {
    if (filteredTree.length > 0) {
      const collectDepth1 = (categories: DbAdminCategory[]): number[] => {
        const ids: number[] = [];
        for (const cat of categories) {
          if (cat.depth === 1) {
            ids.push(cat.id);
  ...
  const findCategoryById = (
    nodes: DbAdminCategory[],
    id: number
  ): DbAdminCategory | undefined => {
  ...
  const selectedCategoryData = selectedCategory
    ...
    : null;
  const isFileCategory = selectedCategoryData?.dbAdminType === "FILE";
  ...
  const handleCategoryDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      await dbAdminApi.reorderCategories(categoryIds, parentId);
      refetchTree();
    } catch (error) {
  ...
  const handleContentDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      await dbAdminApi.reorderContents(selectedCategory, contentIds);
      refetchContents();
    } catch (error) {
  ...
  const renderTreeItem = (cat: DbAdminCategory) => (
    <SortableTreeItem key={cat.id} id={cat.id} isAdminMode={isAdminMode}>
      {({ attributes, listeners }) => (
        <div
          style={{ marginLeft: `${(cat.depth - 1) * 20}px` }}
          className={`group flex items-center gap-1 rounded-md transition-colors ${selectedCategory === cat.id ? "bg-orange-100" : "hover:bg-gray-100"}`}
        >
          ...
          <Button
            ...
            className={`flex-1 justify-start h-8 px-2 hover:bg-transparent ${selectedCategory === cat.id ? "text-orange-900" : ""}`}
          >
            {cat.children && cat.children.length > 0 ? (
              expandedIds.has(cat.id) ? (
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold">
            {project ? `${project} DB Admin` : "All DB Admin"}
          </h1>
        </div>
        <div
          className="flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-200 rounded-full shadow-sm hover:border-orange-200 transition-all cursor-pointer select-none"
          onClick={() => setIsAdminMode(!isAdminMode)}
        >
          <span
            className={`text-xs font-medium ${isAdminMode ? "text-orange-600" : "text-gray-400"}`}
          >
            Admin
          </span>
          <div
            className={`w-8 h-4 rounded-full p-0.5 transition-colors ${isAdminMode ? "bg-orange-600" : "bg-gray-200"}`}
          >
            <div
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="md:col-span-1">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 py-3 bg-orange-100">
            <CardTitle>DB Admin Tree</CardTitle>
          </CardHeader>
          <CardContent>
            {treeLoading ? (
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setContentForm({
                    ...
                    content: "",
                    contentType:
                      (selectedCategoryData?.dbAdminType as any) || "NOTE",
                    metadata: {},
                  });
                            ...
                            <CardHeader className="pb-2">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                  <CardTitle
                                    className="text-lg cursor-pointer hover:text-orange-600 transition-colors flex items-center gap-2 truncate"
                                    onClick={() => {
                              ...
                              <CardContent>
                                ...
                                <div className="mt-2 flex justify-center border-t border-gray-50 dark:border-gray-800 pt-2">
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    className="text-orange-600 hover:text-orange-700 font-medium h-8 w-full"
                                    onClick={() => {
                                      setViewingContent(content);
                                      setIsViewModalOpen(true);
                                    }}
      ...
      <FormDialog
        ...
        onSubmit={async () => {
          if (categoryModalMode === "create") {
            await dbAdminApi.createCategory({
              name: categoryForm.name,
              parentId: categoryForm.parentId ?? undefined,
              dbAdminType: categoryForm.dbAdminType,
              projectType: project || undefined,
            });
          } else if (editingCategory) {
            await dbAdminApi.updateCategory(editingCategory.id, {
              name: categoryForm.name,
              dbAdminType: categoryForm.dbAdminType,
            });
          }
      ...
      <FormDialog
        ...
        onSubmit={async () => {
          if (contentModalMode === "create" && selectedCategory) {
            await dbAdminApi.createContent({
            ...
            await dbAdminApi.updateContent(editingContent.id, {
              title: contentForm.title,
              content: contentForm.content,
              contentType: contentForm.contentType,
              metadata: contentForm.metadata,
            ...
            <Label
              ...
              className={
                contentForm.contentType === "QA"
                  ? "text-orange-600 font-bold text-base block"
            ...
            <div className="mt-2">
              {contentForm.contentType === "QA" ? (
                <Textarea
                  ...
                  rows={4}
                  className="bg-white border-gray-300 focus:border-orange-500 text-lg p-4"
                />
              ) : (
                <Input
                  id="content-title"
                  ...
                  className="bg-white border-gray-300 focus:border-orange-500 h-10"
                />
              )}
            </div>
          </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (categoryToDelete) {
            await dbAdminApi.deleteCategory(categoryToDelete);
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (contentToDelete) {
            await dbAdminApi.deleteContent(contentToDelete);
            refetchContents();
      ...
      <Dialog open={isViewModalOpen} onOpenChange={setIsViewModalOpen}>
        ...
        >
          <DialogHeader className="px-6 py-3 bg-orange-100 border-b border-orange-200">
            <div className="flex items-center justify-between">
          ...
          <div className="flex-1 overflow-y-auto px-8 py-2 bg-white">
            {viewingContent?.contentType === "NOTE" && (
              <div className="prose prose-lg max-w-none dark:prose-invert prose-pre:bg-transparent prose-pre:p-0 prose-pre:m-0 prose-code:text-orange-600 prose-code:before:content-none prose-code:after:content-none font-sans">
                <ReactMarkdown
            ...
            {viewingContent?.contentType === "QA" && (
              <div className="space-y-4">
                {viewingContent.content ? (
                  <div className="prose prose-lg max-w-none dark:prose-invert prose-pre:bg-transparent prose-pre:p-0 prose-pre:m-0 prose-code:text-orange-600 prose-code:before:content-none prose-code:after:content-none">
                    <ReactMarkdown
                      remarkPlugins={[remarkGfm, remarkBreaks]}
                      components={{
                        code: MarkdownCodeBlock as any,
      ...
      <FormDialog
        ...
        onSubmit={async () => {
          if (answerContent) {
            await dbAdminApi.updateContent(answerContent.id, {
              content: answerText,
            });
