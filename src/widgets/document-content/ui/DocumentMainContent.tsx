...
export const DocumentMainContent = () => {
  ...
  const setPanelWidths = useDocumentStore((s) => s.setPanelWidths);
  const reorderTabs = useDocumentStore((s) => s.reorderTabs);
  const moveTabToPanel = useDocumentStore((s) => s.moveTabToPanel);
  const addPanelWithTab = useDocumentStore((s) => s.addPanelWithTab);
  ...
  return (
    ...
    >
      ...
      <DragOverlay>
        {activeTab ? <DocumentTabDragPreview tab={activeTab} /> : null}
      </DragOverlay>
    </DndContext>
  );
};
...
import { DocumentPanel } from "@/widgets/document-sidebar";
...
const DocumentTabContent = ({ panel, isDragging }: DocumentTabContentProps) => {
  const { setNodeRef, isOver } = useDroppable({
    id: `doc-content-${panel.id}`,
...
interface DocumentTabDragPreviewProps {
  tab: DocumentTab;
}

const DocumentTabDragPreview = ({ tab }: DocumentTabDragPreviewProps) => {
  return (
